...
all: kde-platform.tar.gz kde-sdk.tar.gz
...
kde-sdk.tar.gz kde-sdk-rpmdb.tar.gz: $(NOARCH)/kde-sdk-0.1-1.sdk.noarch.rpm
	./setup.sh $(SDK_BASE_IMAGE)
	./build.sh smart install -y  $(NOARCH)/kde-sdk-0.1-1.sdk.noarch.rpm
	rm -rf kde-sdk.tar.gz kde-sdk-rpmdb.tar.gz
	tar --transform 's,^build/root/usr,files,S' -czf kde-sdk.tar.gz build/root/usr --owner=root
	tar --transform 's,^build/var,files,S' -czf kde-sdk-rpmdb.tar.gz build/var/lib/rpm --owner=root
	./clear_root.sh
...
kde-platform-packages: $(NOARCH)/kde-platform-0.1-1.sdk.noarch.rpm $(NOARCH)/freedesktop-platform-base-0.1-1.sdk.noarch.rpm setup.sh build.sh
	./setup.sh $(SDK_BASE_IMAGE)
	rm -f kde-platform-packages
	./build.sh ./list_packages.sh kde-platform > kde-platform-packages
	./clear_root.sh

kde-platform.tar.gz kde-platform-rpmdb.tar.gz: kde-platform-packages $(NOARCH)/kde-platform-0.1-1.sdk.noarch.rpm setup.sh build.sh $(PLATFORM_BASE_IMAGE)
	-echo building kde-platform
	./setup_root.sh $(PLATFORM_BASE_IMAGE)
	./build.sh rpm -Uvh `cat kde-platform-packages`
	tar --transform 's,^build/root/usr,files,S' -czf kde-platform.tar.gz build/root/usr --owner=root
	tar --transform 's,^build/var,files,S' -czf kde-platform-rpmdb.tar.gz build/var/lib/rpm --owner=root
	./clear_root.sh
...
commit-platform: repo kde-platform.tar.gz  kde-platform-rpmdb.tar.gz
	./commit.sh repo kde-platform.tar.gz kde-platform-rpmdb.tar.gz metadata.platform org.gnome.Platform$(EXTRA_NAME) $(ARCH) $(GNOME_VERSION)

commit-sdk: repo kde-sdk.tar.gz kde-sdk-rpmdb.tar.gz
	./commit.sh repo kde-sdk.tar.gz kde-sdk-rpmdb.tar.gz metadata.sdk org.gnome.Sdk$(EXTRA_NAME) $(ARCH) $(GNOME_VERSION)
